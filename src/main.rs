...
async fn main() -> Result<()> {
    ...
    let stat = args.stat.unwrap_or(false);
    let mut last_size = if stat {
        0
    } else {
        log_file.seek(SeekFrom::End(0))?
    };
    let full_size = log_file.metadata()?.len();
    let mut remainder = Vec::new();
    let pb = if !stat {
        None
    } else {
        Some(ProgressBar::new(full_size))
    };
    ...
    loop {
        ...
        if false && lines.is_empty() && remainder.is_empty() && stat {
            break;
        ...
        if stat {
            if let Some(pb) = &pb {
                let current = log_file.stream_position()?;
        ...
        if !stat && print_timer.once() {
            let lines_per_sec = line_count as f64 / f64::from(PRINT_PERIOD);
            let parsed_per_sec = parsed_line_count as f32 / f32::from(PRINT_PERIOD);
            parsed_line_count = 0;
            println!("{lines_per_sec:.1} lines per second\t{parsed_per_sec}");

            // line_count -= round(lines_per_sec * PRINT_PERIOD)
            line_count = line_count
                .saturating_sub(f64::round(lines_per_sec * f64::from(PRINT_PERIOD)) as u32);
        }
    }
    ...
    for doc in docs {
        if let Some(hash) = doc.as_hash() {
            for (name, record_settings) in hash.iter() {
                let Some(name_s) = name.as_str() else {
                    continue;
                };
                let Some(regex) = record_settings["regex"].as_str() else {
                    continue;
                };
...
fn parse_plots(plots_yml: &Yaml, conf: &mut LogRecordType) {
    for (plot_name, plot_conf) in plots_yml.as_hash().unwrap().iter() {
        let Some(name_s) = plot_name.as_str() else {
            continue;
        };
        let axis = plot_conf["axis"]
            .as_i64()
            .and_then(|x| u8::try_from(x).ok());
        let style = plot_conf["style"].as_str();
        let coef = plot_conf["coef"].as_f64();
        let ylim: Option<Vec<f64>> = plot_conf["ylim"].as_vec().and_then(|x| {
            if x.len() == 2 {
                x.iter().map(|y| y.as_f64().as_ref().copied()).collect()
            } else {
                None
            }
        });
        conf.add_field(&name_s, axis, style, coef, ylim);
    }
}
...
async fn get_data(
    ...
    let streams = {
        ...
        let mut filtered_streams: Vec<ParsedBlock> = parsedblocks
            .iter()
...
#[handler]
async fn get_config(config: poem::web::Data<&String>) -> String {
    config.clone()
}
